...
public class FacilityService {
	...
	public Path getFilePath(Integer printerId) {
		Printer printer = facilityRepository.readPrinter(printerId);
		return Paths.get(Define.UPLOAD_FILE_DERECTORY, printer.getUploadFileName());
	}

	public List<Printer> readAllPrinter() {
		return facilityRepository.readAllPrinter();
	}

	public void updatePrinterStatusById(Integer printerId) {
		facilityRepository.updatePrinterStatusById(printerId);
	}

	public void deleteFile(Integer printerId) {
		Printer printer = facilityRepository.readPrinter(printerId);
		File file = new File(Define.UPLOAD_FILE_DERECTORY + printer.getUploadFileName());
		if (file.exists()) {
	        if (file.delete()) {
